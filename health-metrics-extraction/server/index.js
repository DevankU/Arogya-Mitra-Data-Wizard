...
app.post("/api/create-room", (req, res) => {
  ...
  // Verify doctor email
  if (doctorEmail?.toLowerCase() !== DOCTOR_EMAIL.toLowerCase()) {
...
app.post("/api/validate-room", (req, res) => {
  ...
  const isDoctor = email === DOCTOR_EMAIL.toLowerCase();
...
app.get("/api/doctor-rooms", (req, res) => {
  ...
  const doctorRooms = Object.entries(roomInvitations)
    .filter(([_, inv]) => inv.doctorEmail === DOCTOR_EMAIL.toLowerCase())
    .map(([hash, inv]) => ({
      hash,
      roomId: inv.roomId,
...
server.listen(PORT, () => {
  console.log(`MedConsult AI Server running on port ${PORT}`);
