...
export default function ChatDashboard() {
  ...
  useEffect(() => {
    ...
    const checkUser = async () => {
      ...
      const isDoctor = user.email?.toLowerCase() === DOCTOR_EMAIL.toLowerCase();
      ...
      <header className="bg-white/80 backdrop-blur-sm border-b border-[#f4ece7] px-6 py-4 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-[#1c130d] tracking-tight">Airy Peach Health</h1>
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div className={`flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${
                        room.hasPatientJoined 
                          ? 'bg-emerald-50 text-emerald-700' 
                          : 'bg-amber-50 text-amber-700'
                      }`}>
                        <span className={`w-2 h-2 rounded-full ${
                          room.hasPatientJoined ? 'bg-emerald-500' : 'bg-amber-500 animate-pulse'
                        }`}></span>
                    ...
                    <div className="flex items-center gap-4 mb-4">
                      <div className={`w-12 h-12 rounded-full flex items-center justify-center ${
                        room.hasPatientJoined ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'
                      }`}>
                        <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
